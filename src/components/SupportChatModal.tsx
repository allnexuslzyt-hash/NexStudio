...
export const SupportChatModal: React.FC = () => {
  ...
  // Tab state: 'tickets' (Human Support Tickets, default/priority) vs 'jaime' (Virtual Assistant quick help)
  const [activeTab, setActiveTab] = useState<'tickets' | 'jaime'>('tickets');

  // Switch to tickets tab if activeTicketId is populated
  React.useEffect(() => {
    if (activeTicketId) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/60 backdrop-blur-xs">
      ...
      >
              ...
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                {activeTab === 'tickets' && activeTicket ? (
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 flex items-center gap-1">
                    ...
                    <span>Asistencia Rápida IA</span>
                  </span>
                ) : null}
              </h3>
              <p className="text-[11px] text-slate-500">
                {activeTab === 'jaime' 
                  ? 'Asistencia instantánea con Jaime IA · Para atención con un administrador, usa los tickets de soporte' 
        ...
        {/* Support Navigation Tabs (Tickets have preference/priority) */}
              ...
              <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 border-r border-slate-200">
                {!user ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
                    ...
                    <div className="pt-2 border-t border-slate-100 w-full max-w-xs flex flex-col items-center gap-1.5">
                      <span className="text-[11px] text-slate-400">¿Tienes una consulta general o rápida?</span>
                      <button
                        type="button"
                        onClick={() => setActiveTab('jaime')}
                      ...
                      >
                        ...
                        <span>Consultar Asistencia Rápida con Jaime IA</span>
                      </button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    {/* Quick assistance suggestion banner */}
                    <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 flex items-center justify-between gap-3 text-xs">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="font-semibold text-slate-800 text-[11px]">¿Duda rápida sobre NexClean o la plataforma?</p>
                          <p className="text-[10px] text-slate-500">Obtén respuesta inmediata con la Asistencia Rápida (IA).</p>
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => setActiveTab('jaime')}
                        className="shrink-0 px-2.5 py-1 rounded-lg bg-white hover:bg-slate-100 text-indigo-600 font-semibold text-[11px] border border-slate-200 shadow-2xs transition-colors cursor-pointer"
                      >
                        Preguntar a Jaime
                      </button>
                    </div>
